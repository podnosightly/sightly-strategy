---
name: sightly-proposal-deck-builder
description: >
...
**Trust the data, attribute third-party figures on-slide.** Stats the strategist provides from Brand Mentality(R) outputs are pre-verified as real and correctly measured. Use them directly; do not hold them out or hedge them. The one thing that survives is attribution, which is a credibility question, not a verification one. When a figure is third-party in origin (viewership numbers, subscriber counts, platform view counts, external research that Brand Mentality(R) surfaced rather than generated), carry its source label on the slide so it holds up under a skeptical agency read. Sightly's own proof points (the MAGNA 2X search and 3X purchase intent, the +20% engagement lift) already carry their attribution and are drop-in ready. Never invent a figure or a source; if a line needs a number that was not provided, flag the gap rather than filling it.
...
**Hard Rules terminology, every slide.** Brand Mentality(R), Anticipation Software(R), SmartLists(TM) with their marks. "Brand Suitability" not "brand safety." "Social Intelligence" not "social listening." Never name competitors (use "other offerings in the market"). Never name white-label partners. Never use "programmatic" in Spotify materials. IRIS.TV, Spectrum Reach, and Transmit Live can be named, always leading with Sightly. Never frame Sightly's size as a limitation. Never position Sightly as insight-only; it is always activation-led.
